...
def fetch_section_image_ids(
	...
	Args:
	    atlas_id: Atlas ID (1=Mouse P56 Coronal, 2=Mouse P56 Sagittal).
	    group_id: Graphic group ID (28=structure boundaries).
	    timeout: Request timeout in seconds.

	Returns:
	    List of section IDs (sub_images.id).
	"""
	url = (
	...
	if df.empty:
		raise AllenAPIError(
			f"Empty response for section IDs.\nURL: {url}\nPreview:\n{r.text[:500]}"
		)
	...
	if "sub_images.id" in df.columns:
		...
		if not int_cols:
			raise AllenAPIError(
				f"Could not find an integer ID column.\nColumns: {list(df.columns)}"
			)
		col = int_cols[0]
...
def download_section_svg(
	...
	*,
	...
	Args:
	    section_image_id: Allen section image (sub_image) ID.
	    group_id: Boundary group ID defining which structures are included
	                   (default: 28, structure boundaries).
	    timeout: Maximum time (seconds) to wait for the Allen API response.
